...
from NGrams import *  # Module for storing nGram time histories


class WordSwarm(Framework):
	# Settings
	name = "Animated Word Cloud: Research trends in economics"
	saveFrames = True
	csvName = 'input_data.csv'
		...
		for word_k in range(0, self.nGrams.nWords):
			...
			if phase == 1:
				...
				oldSize[1] = (self.maxSize - self.minSize) * (
							self.nGrams.counts[word_k][date_k - 118] / float(self.nGrams.maxCount)) + self.minSize
